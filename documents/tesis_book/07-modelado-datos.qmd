...
#| tbl-cap: "Acuerdo entre sistemas de anotación por dimensión."
...
Para las dimensiones categóricas el panorama es más heterogéneo. Estrategia presenta la coincidencia bruta más alta (61.2%) pero un kappa bajo ($\kappa = 0.126$), lo cual refleja una distribución desbalanceada donde la categoría mayoritaria infla la coincidencia sin elevar el acuerdo ajustado por azar. Frontera muestra un kappa negativo ($\kappa = -0.146$). **Este valor negativo no significa que los modelos diverjan al azar: significa que discrepan de manera sistemática.** Ambos sistemas tienden a identificar los mismos comentarios como fronterizos, pero discrepan de forma consistente sobre la *dirección* de esa frontera ---si es entre bloques o al interior del bloque de derecha---, lo que genera un patrón de desacuerdo no aleatorio que el estadístico kappa penaliza con valores negativos. En términos sustantivos, esto señala que la distinción inter/intra-bloque es la dimensión más difícil de operacionalizar automáticamente, lo cual es consistente con la ambigüedad pragmática de comentarios que pueden leerse simultáneamente como crítica interna a la derecha y como refuerzo de la frontera con la izquierda. Marco y emoción registran los acuerdos más bajos ($\kappa = 0.136$ y $\kappa = 0.076$, respectivamente), consistente con la mayor ambigüedad inherente a estas categorías: un mismo comentario admite lecturas legítimas en más de una categoría (por ejemplo, como indignación o desprecio, o encuadrado en un marco de conflicto o diagnóstico), sin que ninguna de las dos lecturas sea incorrecta.
...
```{r tbl-cv-regresion}
#| tbl-cap: "Modelos de regresión para polarización."
