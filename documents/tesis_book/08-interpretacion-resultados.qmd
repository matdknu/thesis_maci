...
el ciclo electoral refuerza pero no origina.

```{r fig-kast-trans}
#| results: asis
#| echo: false
if (file.exists(file.path(base_out, "transiciones_autor_kast_fases.png"))) {
  cat('![Transiciones de sentimiento hacia Kast por autor (PNG).](fig_thesis/transiciones_autor_kast_fases.png){#fig-kast-trans fig-align="center" width="82%"}\n')
}
```
...
lectura se centre en polaridad clara sin diluir con neutralidad.

```{r tbl-cruce-sentimiento-pares-sv}
cruce_path <- file.path(base_out, "cruce_segunda_vuelta_sentimiento_kast_jara_pct_sin_neutro.csv")
if (file.exists(cruce_path)) {
  cruce <- utils::read.csv(cruce_path, stringsAsFactors = FALSE, check.names = FALSE, encoding = "UTF-8")
  cruce <- cruce[order(cruce[["Par"]], cruce[[2]], cruce[[3]]), ]
  kable_apa(
    cruce,
    caption = paste(
      "Probabilidad condicional de tono en segunda vuelta (hacia Kast o Jara) dado el tono de posicionamiento hacia el candidato de origen,",
      "seis pares; sin tono neutro en origen ni destino (porcentajes renormalizados).",
      "Agregación por autor-fase; conteos técnicos en `outputs_ml/cruce_posicionamiento_segunda_vuelta_pares_candidatos.csv`."
    ),
    note = NULL,
    align = "lllr",
    landscape = FALSE,
...
```{r tbl-cruce-emocion-sin-se-pares-sv}
...
if (file.exists(emo_path)) {
  ...
  kable_apa(
    emo,
    caption = paste(
      "Mismo diseño que la tabla anterior, con grupos emocionales (`emocion_final` agregada por moda de autor-fase);",
...
```{r fig-alluvial-cruces-sv}
#| results: asis
#| echo: false
if (file.exists(file.path(base_out, "alluvial_cruces_segunda_vuelta_por_candidato_origen.png"))) {
  cat('![Diagramas tipo Sankey/alluvial: tono en posicionamiento (izquierda) hacia el candidato de origen del subtítulo, y tono en segunda vuelta hacia Kast o Jara (derecha). Grosor proporcional al porcentaje del total de autores con ese tono hacia el origen (100 % por nodo izquierdo).](fig_thesis/alluvial_cruces_segunda_vuelta_por_candidato_origen.png){#fig-alluvial-cruces-sv fig-align="center" width="100%"}\n')
}
```
...
Figura generada con `scripts/analisis/16_alluvial_cruces_segunda_vuelta.py`.
...
if (file.exists(file.path(base_out, "scatter_actividad_polarizacion_autores.png"))) {
  cat('![Actividad (número de comentarios) y polarización media por autor: color según candidato con más menciones en sus comentarios (empate en violeta; sin candidato reconocible en gris). Ligero desplazamiento horizontal de los puntos solo para visualizar solapamientos.](fig_thesis/scatter_actividad_polarizacion_autores.png){#fig-autores-scatter fig-align="center" width="82%"}\n')
}
```
...
participación política. Como resume la @tbl-kast-transiciones, la gran
