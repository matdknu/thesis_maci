...
En este apartado conviene distinguir dos agregaciones: la **frontera política** del comentario (inter-bloque, intra-bloque o sin frontera) y el **candidato objetivo** al que se dirige el mensaje. La @tbl-estrategia-frontera cuantifica la primera: proporción de cada estrategia discursiva según tipo de frontera. La distribución por candidato se incluye en el Anexo A (@fig-anexo-estrategias-candidato).

Esa lectura por frontera revela un patrón
...
Las combinaciones entre marco y emoción (valores numéricos en el Anexo A, @tbl-anexo-marco-emocion-top; visualización en @fig-marco-emocion) permiten ir más allá de las distribuciones
...
if (file.exists(file.path(base_out, "combinaciones_marco_emocion.png"))) {
  cat('![Combinaciones marco × emoción.](fig_thesis/combinaciones_marco_emocion.png){#fig-marco-emocion fig-align="center" width="82%"}\n')
}
```
...
![Evolución semanal de estrategias discursivas.](fig_thesis/serie_estrategias_semanal_mejorada.png){#fig-estrategias-temporal
...
En la segunda vuelta solo compiten Kast y Jara, por lo que el destino del cruce es siempre el tono hacia uno de ellos en esa fase. La tabla de cruces de tono (sin neutro) figura en el Anexo A (@tbl-anexo-cruce-sentimiento-pares-sv). La tabla de cruces emocionales figura en el Anexo A (@tbl-anexo-cruce-emocion-sin-se-pares-sv). La @fig-alluvial-emocion-sv resume los flujos en formato diagrama.
...
if (file.exists(file.path(base_out, "alluvial_emocion_segunda_vuelta_por_candidato_origen_sin_sin_emocion.png"))) {
  cat('![Flujos emocionales entre posicionamiento y segunda vuelta.](fig_thesis/alluvial_emocion_segunda_vuelta_por_candidato_origen_sin_sin_emocion.png){#fig-alluvial-emocion-sv fig-align="center" width="92%"}\n')
}
```
...
if (file.exists(file.path(base_out, "polarizacion_por_tipo_autor.png"))) {
  cat('![Polarización media por tipo de autor.](fig_thesis/polarizacion_por_tipo_autor.png){#fig-autores-violin fig-align="center" width="82%"}\n')
}
```
...
if (file.exists(file.path(base_out, "scatter_actividad_polarizacion_autores.png"))) {
  cat('![Actividad y polarización media por autor.](fig_thesis/scatter_actividad_polarizacion_autores.png){#fig-autores-scatter fig-align="center" width="82%"}\n')
}
```
...
El atlas (@fig-3d-atlas-global-v1) ofrece una lectura visual que complementa los hallazgos estadísticos anteriores. El panel de candidatos (superior derecho) muestra una superposición casi completa de los cuatro colores: no existen regiones exclusivas por candidato. Desde la perspectiva de Mouffe, esto sugiere que la frontera política no se traza entre vocabularios diferenciados, sino a través de un repertorio léxico compartido — la polarización es una propiedad del discurso, no del target.
...
if (file.exists(path_atlas_global)) {
  cat('![Atlas global 3D: temas, candidatos, polarización y emociones.](fig_thesis/3d_atlas_v1_globales.png){#fig-3d-atlas-global-v1 fig-align="center" width="100%"}\n')
} else {
